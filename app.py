...
def show_login():
    ...
    username_entry.pack(fill="x", pady=(6, 16))
    ...
    password_entry.pack(fill="x", pady=(6, 24))
    ...
    def login():
        ...
        if not username or not password:
            show_error("Login", "Please enter your username and password.")
            return
...
def show_workspace():
    global current_truck_id, current_plate, workspace_logo_image
    ...
    header = tk.Frame(root, bg=WHITE, height=92)
    header.pack(fill="x")
    header.pack_propagate(False)

    title_frame = tk.Frame(header, bg=WHITE)
    title_frame.pack(side="left", fill="y", padx=28)
    ...
    if workspace_logo_image:
        ...
        logo_label.pack(side="right", padx=25)
    ...
    content.pack(fill="both", expand=True, padx=22, pady=(12, 12))
    ...
    truck_section = tk.Frame(top_bar, bg=WHITE)
    truck_section.pack(side="left", padx=14, pady=11)
    ...
    truck_combo.grid(row=1, column=0, padx=(0, 8), pady=(4, 0))
    ...
    ).grid(row=1, column=2, padx=3, pady=(4, 0))
    ...
    ).grid(row=1, column=3, padx=3, pady=(4, 0))

    search_section = tk.Frame(top_bar, bg=WHITE)
    search_section.pack(side="right", padx=14, pady=11)
    ...
    def perform_search():
        load_records(search_entry.get().strip())
    ...
    previous_odo_var = tk.StringVar(value="N/A")
    current_odo_var = tk.StringVar()
    distance_var = tk.StringVar(value="N/A")
    efficiency_var = tk.StringVar(value="N/A")
    ...
    for label_text, variable, column in fields:
        cell = tk.Frame(form_frame, bg=WHITE)
        cell.grid(
            row=0,
            column=column,
            padx=6,
        ...
        entry = ttk.Entry(cell, textvariable=variable, width=16)
        ...
        form_frame.grid_columnconfigure(column, weight=1)

    # Calculated values
    calc_frame = tk.Frame(form_frame, bg="#f8fafc")
    calc_frame.grid(
        row=1,
        column=0,
        columnspan=7,
    ...
    tk.Label(
        ...
        fg=TEXT,
        bg="#f8fafc"
    ).grid(row=0, column=1, padx=(0, 30), pady=8, sticky="w")
    ...
    def calculate_odo(*args):
        try:
            ...
            try:
                liters = float(liters_var.get().strip())

                if liters <= 0:
                    efficiency_var.set("N/A")
                else:
                    efficiency = distance / liters
                    efficiency_var.set(f"{efficiency:,.2f} km/L")
    ...
    current_odo_var.trace_add("write", calculate_odo)
    liters_var.trace_add("write", calculate_odo)
    ...
    def save_record():
        ...
        if current_truck_id is None:
            show_error("Save Record", "Please select a truck first.")
            return
        ...
        try:
            current_odo = float(current_odo_text)
        except ValueError:
            show_error(
                "Invalid ODO",
        ...
        previous_odo = get_previous_odo(current_truck_id)

        if previous_odo is not None and current_odo < previous_odo:
            show_error(
                "Invalid ODO",
                f"Current ODO cannot be lower than Previous ODO "
                f"({previous_odo:,.2f})."
            )
            return
        ...
        try:
            ...
            # Keep the latest saved record visible at the top.
            if record_tree.get_children():
                record_tree.selection_set(record_tree.get_children()[0])
                record_tree.focus(record_tree.get_children()[0])
                record_tree.see(record_tree.get_children()[0])
    ...
    # Excel-style Enter behavior while encoding:
    # - Enter moves to the next input field.
    # - Enter on the last field (Current ODO) saves the record.
    # - Shift+Enter moves to the previous field.
    # - KP_Enter (numeric keypad Enter) behaves the same way.
    encoding_order = [
        "ATW Number",
        "Destination",
        "Liters",
    ...
    def encoding_enter(event):
        widget = event.widget
        current_name = next(
            (name for name, entry in entries.items() if entry is widget),
            None
        )
        ...
        index = encoding_order.index(current_name)

        # Shift+Enter goes to the previous field.
        if event.state & 0x0001:
            if index > 0:
                entries[encoding_order[index - 1]].focus_set()
            return "break"

        # Enter on the last field saves the record.
    ...
    for name in encoding_order:
        entries[name].bind("<Return>", encoding_enter)
        entries[name].bind("<KP_Enter>", encoding_enter)
    ...
    table_title = tk.Frame(table_frame, bg=WHITE)
    table_title.pack(fill="x", padx=12, pady=(9, 5))
    ...
    for column in columns:
        record_tree.heading(column, text=headings[column])
        record_tree.column(
            column,
            width=widths[column],
    ...
    record_tree.column("destination", anchor="w")
    ...
    record_tree.tag_configure("even", background="#f9fafb")
    record_tree.tag_configure("odd", background="#ffffff")

    # -----------------------------------------------------
    # TABLE ACTIONS + LOGOUT
    # -----------------------------------------------------
    ...
    def refresh_previous_odo():
        if current_truck_id is None:
            previous_odo_var.set("N/A")
        else:
            previous = get_previous_odo(current_truck_id)

            if previous is None:
                previous_odo_var.set("N/A")
            else:
                previous_odo_var.set(f"{previous:,.2f}")
    ...
    def load_records(search_text=""):
        ...
        try:
            ...
            # Oldest record gets #1, newest gets the highest number.
            ...
            for row_index, row in enumerate(rows):
                ...
                date_value = row[2]
                destination = row[3]
                liters = float(row[4]) if row[4] is not None else 0
                ...
                if previous is not None and current is not None:
                    distance = float(current) - float(previous)
    ...
    def add_truck():
        ...
        plate_entry = ttk.Entry(dialog, width=32)
        plate_entry.pack(pady=7)
    ...
    def edit_truck():
        ...
        if current_truck_id is None:
            show_error("Edit Truck", "Please select a truck.")
            return

        dialog = tk.Toplevel(root)
        dialog.title("Edit Truck")
        ...
        plate_entry = ttk.Entry(dialog, width=32)
        plate_entry.insert(0, current_plate)
        plate_entry.pack(pady=7)
        plate_entry.focus_set()
        plate_entry.select_range(0, "end")
        ...
        def save_edit():
            ...
            if not plate:
                show_error("Edit Truck", "Please enter a plate number.")
                return
    ...
    def delete_truck():
        global current_truck_id, current_plate

        if current_truck_id is None:
            show_error("Delete Truck", "Please select a truck.")
            return

        if not messagebox.askyesno(
            "Delete Truck",
            f"Delete truck {current_plate} and all of its records?"
        ):
            return
    ...
    def load_trucks(select_plate=None):
        global current_truck_id, current_plate, truck_map
        ...
        try:
            ...
            plates = [plate for _, plate in rows]
            ...
            target = select_plate if select_plate in plates else plates[0]

            truck_var.set(target)
            current_plate = target
            current_truck_id = truck_map[target]
    ...
    def change_truck(event=None):
        ...
        current_plate = plate
        current_truck_id = truck_map.get(plate)

        clear_form()
        load_records()

    truck_combo.bind("<<ComboboxSelected>>", change_truck)
    ...
    def edit_record():
        ...
        if not record:
            show_error("Edit Record", "Record not found.")
            return
        ...
        edit_date = tk.StringVar(
            value=record[1].strftime("%Y-%m-%d")
            if hasattr(record[1], "strftime")
            else str(record[1])
        )
        edit_atw = tk.StringVar(value=str(record[0]))
        edit_destination = tk.StringVar(value=str(record[2]))
        edit_liters = tk.StringVar(value=str(record[3]))
        edit_amount = tk.StringVar(value=str(record[4]))
        edit_current_odo = tk.StringVar(value=str(record[5]))

        edit_previous_odo = None

        # Previous ODO = most recent record before this record in ID sequence.
        db2 = None
        cursor2 = None
        ...
        try:
            ...
            cursor2.execute(
                ...
                LIMIT 1
                """,
                (current_truck_id, record_id)
            )

            result = cursor2.fetchone()

            if result:
                edit_previous_odo = float(result[0])
        ...
        edit_distance = tk.StringVar(value="N/A")
        edit_efficiency = tk.StringVar(value="N/A")
        ...
        for row_index, (label_text, variable) in enumerate(edit_fields):
            tk.Label(
                form,
                text=label_text,
                font=("Segoe UI", 9, "bold")
        ...
        tk.Label(
            ...
            font=("Segoe UI", 9, "bold"),
            bg="#f8fafc"
        ).grid(row=0, column=0, padx=(10, 4), pady=9)
        ...
        tk.Label(
            ...
            font=("Segoe UI", 9),
            bg="#f8fafc"
        ).grid(row=0, column=1, padx=(0, 25), pady=9)
        ...
        tk.Label(
            ...
            font=("Segoe UI", 9, "bold"),
            bg="#f8fafc"
        ).grid(row=0, column=2, padx=(0, 4), pady=9)
        ...
        tk.Label(
            ...
            font=("Segoe UI", 9),
            bg="#f8fafc"
        ).grid(row=0, column=3, padx=(0, 10), pady=9)

        def calculate_edit():
            try:
                current = float(edit_current_odo.get().strip())

                if edit_previous_odo is None:
                    edit_distance.set("N/A")
                    edit_efficiency.set("N/A")
                    return

                if current < edit_previous_odo:
                    edit_distance.set("Invalid ODO")
                    edit_efficiency.set("Invalid ODO")
                    return

                distance = current - edit_previous_odo
                edit_distance.set(f"{distance:,.2f}")

                liters = float(edit_liters.get().strip())
        ...
        edit_current_odo.trace_add("write", lambda *args: calculate_edit())
        edit_liters.trace_add("write", lambda *args: calculate_edit())

        def update_record():
            date_text = edit_date.get().strip()
            atw = edit_atw.get().strip()
            destination = edit_destination.get().strip()
            liters_text = edit_liters.get().strip()
            amount_text = edit_amount.get().strip()
            current_odo_text = edit_current_odo.get().strip()
